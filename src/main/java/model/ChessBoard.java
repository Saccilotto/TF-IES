...
public class ChessBoard {
	...
	public boolean canCastle(boolean t, char side) {
		...
		if (t == WHITE && side == 'K') {
			...
			if (whiteKingMoved || whiteQRookMoved || isOccupied(new Point(0, 3)) || isOccupied(new Point(0, 2))
			...
			if (blackKingMoved || blackKRookMoved || isOccupied(new Point(7, 5)) || isOccupied(new Point(7, 6)))
				value = false;
			else
				value = true;
		} else if (t == BLACK && side == 'Q') {
			if (blackKingMoved || blackQRookMoved || isOccupied(new Point(7, 3)) || isOccupied(new Point(7, 2))
